                    ...
                    Some(channel) => {
                        let res = match channel.value() {
                            Channel::TextChannel { server, .. } => {
                                check_for_invites(server, &message, &bot, &user, &linkRipper).await
                            }
...
async fn handle_command(
    ...
    if let Some(msg) = &message.content {
        if msg.starts_with("observer ") {
                    ...
                    match user.http.join_invite(id.clone()).await {
                        Ok(joinDat) => {
                            ...
                            logToNotes(
                                ...
                                SendMessageData {
                                    content: Some(
                                        "Started listening via invite: INVID".replace("INVID", &id),
                                    ),
                                    ..Default::default()
                                },
                            )
                        ...
                        Err(e) => {
                            ...
                            logToNotes(
                                ...
                                SendMessageData {
                                    content: Some(
                                        "Failed to join invite: INVID".replace("INVID", &id),
                                    ),
                                    ..Default::default()
                                },
                            )
                    ...
                    match user.http.delete_or_leave_server(id.clone()).await {
                        Ok(joinDat) => {
                            ...
                            logToNotes(
                                ...
                                SendMessageData {
                                    content: Some(
                                        "Stopped listening in: INVID".replace("INVID", &id),
                                    ),
                                    ..Default::default()
                                },
                            )
            ...
            } else if cmd.starts_with("forceIndex")
                && tryIndexInviteFromMessage(&linkRipper, cmd, &bot, &user)
                    .await
                    .is_ok()
            {
                informOwner(
...
async fn tryIndexInviteFromMessage(
    ripper: &Regex,
    msg: &str,
    bot: &Rive,
    user: &Rive,
    ...
    return match ripper.captures(&msg) {
        Some(invites) => {
            ...
            match bot
                .http
                .send_message("01HQS9NN019MR8RHN2VHG259WB", data.clone())
                .await
            {
                Ok(_) => Ok(()),
                Err(e) => {
